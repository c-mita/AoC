...
grid_power = {(x, y):cell_power(x, y, s_value) for x in xrange(1, 301) for y in xrange(1, 301)}

subgrids = [(x, y) for x in xrange(1, 298) for y in xrange(1, 298)]

best_grid = max(subgrids, key=lambda (x0, y0): sum(grid_power[(x, y)] for x in xrange(x0, x0+3) for y in xrange(y0, y0+3)))
print best_grid
